...
export default class Completed extends Component {
  ...
  Completed = () => {
    ...
    AsyncStorage.getItem("pagelimit").then(pagelimit => {
      ...
      console.log(API.m_call_completed);
      console.log(JSON.stringify(Request));
      NetInfo.fetch().then(state => {
        if (state.isConnected) {
          timeout(
            15000,
            fetch(API.m_call_completed, {
              method: "POST",
              headers: {
              ...
              .then(res => {
                if(res.status == 200){
                 ...
                 res.json() .then(res => {
                console.log("m_completed :::  ", res);
                if (res.status == "success") {

                  this.setState({loading:false,loading1: false, CompletedData: this.state.CompletedData.concat(res.data) , radioItems: res.sort_by,  page: parseInt(this.state.page) + parseInt(pagelimit)})   
