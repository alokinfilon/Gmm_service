...
export default class SpareRecommended extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
        ...
        AsyncStorage.getItem("token").then(token => {
          AsyncStorage.getItem("branch_id").then(branch_id => {
            AsyncStorage.getItem("pagelimit").then(pagelimit => {
              ...
              console.log(API.spare_recommended);
              console.log(JSON.stringify(Request));
              NetInfo.fetch().then(state => {
                if (state.isConnected) {
                  timeout(
                    15000,
                    fetch(API.spare_recommended, {
                      method: "POST",
                      headers: {
  ...
  render() {
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: Colors.primary }}>
        ...
        <Header
          backIcon={this.props.navigation.state.params.Navigate == 'Home'? require('../images/Left_arrow.png'):require('../images/menu.png')}
            pageTitle="Spare Recommended"
            back={() => {
...
<Text
                      ...
                      <Text style={styles.label}>Contact Person</Text>
                        <View style={{ flex: 1, flexDirection: 'column' }}>
                          <Text style={styles.value}>{item.caller_name ? item.caller_name: "-"}</Text>
                        </View>
                      </View>
                      ...
                      <Text style={styles.label}>Service Engineer</Text>
                        <View style={{ flex: 1, flexDirection: 'column' }}>
                      <Text style={styles.value}>{item.user} {item.user_type == 2 ? "- Manager" : "- Engineer"}</Text>
                        </View>
                      </View>
                      ...
                      <Text style={styles.label}>Requested Date/Time</Text>
                        <View style={{ flex: 1, flexDirection: 'column' }}>
                          <Text style={styles.value}>{moment(item.spare_date).format("DD/MM/YYYY hh:mm a")}</Text>
                        </View>
                      </View>

                      <View style={styles.rowItem}>
                      <Text style={styles.label}>Spare Required</Text>
                        <View style={{ flex: 1, flexDirection: 'column' }}>
                          <Text style={styles.value}>{item.spare_name}</Text>
                        </View>
                      </View>
...
//                 <View
//             style={{zIndex: 1, bottom: -20,right:-30, position: 'absolute', opacity:0.2}}>
//                  <Image source={require('../images/primaryfill.png')} style={{height:120, width:120, tintColor: Colors.primary}} />
...
//               // this.props.navigation.navigate('UploadEX')
//               }}>
//             <ImageBackground
//               resizeMode="contain"
//               style={{
