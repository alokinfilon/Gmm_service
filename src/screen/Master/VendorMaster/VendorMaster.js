        ...
        AsyncStorage.getItem("token").then(token => {
          AsyncStorage.getItem("branch_id").then(branch_id => {
            AsyncStorage.getItem("pagelimit").then(pagelimit => {
              ...
              console.log(API.vendor_get_list);
              console.log(JSON.stringify(Request));
              NetInfo.fetch().then(state => {
                if (state.isConnected) {
                  timeout(
                    15000,
                    fetch(API.vendor_get_list, {
                      method: "POST",
                      headers: {
                      ...
                      .then(res => {
                        if (res.status == 200) {
                          ...
                          res.json().then(res => {
                            console.log("vendor_get_list :::  ", res);
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: Colors.primary }}>
        <View style={styles.container}>
          ...
          <Header
            ...
            press={() =>
              this.props.navigation.navigate('AddVendorMaster', { screen: 'view' })
            }
          />
              ...
              renderItem={({ item, index }) => (
                <View
                  ...
                  {this.state.type_id == 1 ?
                    <TouchableOpacity
                      onPress={() => {
                        this.props.navigation.navigate('AddVendorMaster', { screen: 'edit', item: item })
                      }}
