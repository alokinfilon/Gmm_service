...
export default class Future extends Component {
  ...
  ClearFilter = () => {
    ...
    date = "";
    this.setState({
      page: 0, futureData: [], search: '', loading1: false, grp: '',
      grpId: ''
    }, () => {
      setTimeout(() => {
        this.Future();
  ...
  Future = () => {
    if (futureArray.includes(this.state.page)) {

    } else {
      if (futureArray.length < 1) {
        this.setState({ loading: true });
      } else {
        this.setState({ loading1: true });
      }
      futureArray.push(this.state.page)
        ...
        AsyncStorage.getItem("token").then(token => {
          AsyncStorage.getItem("branch_id").then(branch_id => {
            AsyncStorage.getItem("pagelimit").then(pagelimit => {
              ...
              console.log(API.e_call_list_future);
              console.log(JSON.stringify(Request));
              NetInfo.fetch().then(state => {
                if (state.isConnected) {
                  timeout(
                    15000,
                    fetch(API.e_call_list_future, {
                      method: "POST",
                      headers: {
                      ...
                      .then(res => {
                        if (res.status == 200) {
                          ...
                          if (this.state.grp || date) {
                            this.setState({ FilterVisible: true, refresh: !this.state.refresh })
                          } else {
                            this.setState({ FilterVisible: false, refresh: !this.state.refresh })
                          }
                          res.json().then(res => {
                            console.log("m_future :::  ", res);
                            if (res.status == "success") {
                              this.setState({
                                loading: false, loading1: false, futureData: this.state.futureData.concat(res.data),
                                //  radioItems: res.sort_by,
                                grpData: res.call_group, page: parseInt(this.state.page) + parseInt(pagelimit)
                              })
  ...
  _handleDatePicked = date0 => {
    ...
    { this.state.loading }
    this.setState({ page: 0, futureData: [], loading1: false }, () => {
      futureArray = [];
      this.Future()
    });
  };
  ...
  changeActiveRadioButton(index) {
    ...
    futureArray = [];
    this.setState({ order_field: this.state.radioItems[index].value, modalVisible: false, page: 0, futureData: [] }, () => {
      setTimeout(() => {
        this.Future();
      }, 500)
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (this.state.sort_direction == 'asc') {
            futureArray = [];
            this.setState({
              sort_direction: 'desc', page: 0, futureData: [],
            }, () => {
              this.Future()
            });
          } else {
            futureArray = [];
            this.setState({
              sort_direction: 'asc', page: 0, futureData: [],
            }, () => {
              this.Future()
            });
          }
          ...
          <View
            ...
            <TextInput
              ...
              onSubmitEditing={() => {
                ...
                this.setState({ page: 0, futureData: [] }, () => {
                  futureArray = [];
                  this.Future()
                });
              }}
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        {this.state.page == 0 && this.state.futureData.length < 1 && this.state.loading == false && this.state.loading1 == false ?
          ...
          <FlatList
            ...
            onEndReached={() => {
              AsyncStorage.getItem("pagelimit").then((pagelimit) => {
                if (pagelimit) {
                  if (this.state.futureData.length >= pagelimit) {
                    this.handleLoadMore();
                  }
                }else{
                  this.handleLoadMore();
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    NetInfo.fetch().then(state => {
                      if (state.isConnected) {
                        this.props.navigation.navigate('EmpRunning', { item: item })
                      } else {
                        Toast.show(
                          'Please Check your internet connection',
                          Toast.SHORT,
        ...
        {this.state.FilterVisible ?
          <TouchableOpacity style={{ width: '100%', height: height * 0.07, marginTop: 20 }}
            onPress={() => {
              NetInfo.fetch().then(state => {
                if (state.isConnected) {
                  this.ClearFilter()
            ...
            <View style={{ flex: 1, backgroundColor: Colors.primary, borderRadius: 0 }}>
              <ImageBackground
                resizeMode="cover"
                style={{top:4, flex: 0.3, height: 40, width: 40, marginHorizontal: 5, marginVertical: 0.5 }}
                source={require('../../images/fill.png')}>
                <Icon
                  name="filter"
                  size={20}
                  color={Colors.primary}
                  style={{ height: 18, width: 18, tintColor: Colors.primary, marginVertical: 12, marginHorizontal: 11 }}
                />
              </ImageBackground>
              <Text
                style={{
                            ...
                            <View>
                              <Dropdown
                                ...
                                onChangeText={(value, id) => {
                                  this.setState({ grp: value, grpId: id }, () => {
