...
export default class Completed extends Component {
  ...
  ClearFilter = () => {
    ...
    date = "";
    this.setState({
      page: 0, completedData: [], search: '', loading1: false, grp: '',
      grpId: ''
    }, () => {
      setTimeout(() => {
        this.Completed();
  ...
  Completed = () => {
    if (completedArray.includes(this.state.page)) {
      ...
      completedArray.push(this.state.page)
        ...
        AsyncStorage.getItem("token").then(token => {
          AsyncStorage.getItem("branch_id").then(branch_id => {
            AsyncStorage.getItem("pagelimit").then(pagelimit => {
              ...
              console.log(API.e_call_list_completed);
              console.log(JSON.stringify(Request));
              NetInfo.fetch().then(state => {
                if (state.isConnected) {
                  timeout(
                    15000,
                    fetch(API.e_call_list_completed, {
                      method: "POST",
                      headers: {
                        ...
                        if (res.status == 200) {
                          ...
                          res.json().then(res => {
                            ...
                            if (res.status == "success") {
                              this.setState({
                                loading: false, loading1: false, completedData: this.state.completedData.concat(res.data),
  ...
  _handleDatePicked = date0 => {
    ...
    { this.state.loading }
    this.setState({ page: 0, completedData: [], loading1: false }, () => {
      completedArray = [];
      this.Completed()
    });
  };
  ...
  setModalVisible(visible) {
    if (this.state.completedData.length < 1) {
      Toast.show(
        "No More completed call found...",
        Toast.SHORT,
      );
    } else {
  ...
  changeActiveRadioButton(index) {
    ...
    completedArray = [];
    this.setState({ order_field: this.state.radioItems[index].value, modalVisible: false, page: 0, completedData: [] }, () => {
      setTimeout(() => {
        this.Completed();
      }, 500)
    });
      ...
      <TouchableOpacity
        ...
        onPress={() => {
          if (this.state.sort_direction == 'asc') {
            completedArray = [];
            this.setState({
              sort_direction: 'desc', page: 0, completedData: [],
            }, () => {
              this.Completed()
            });
          } else {
            completedArray = [];
            this.setState({
              sort_direction: 'asc', page: 0, completedData: [],
            }, () => {
              this.Completed()
            });
          }
          ...
          <View
            ...
            <TextInput
              ...
              onSubmitEditing={() => {
                ...
                this.setState({ page: 0, completedData: [] }, () => {
                  completedArray = [];
                  this.Completed()
                });
              }}
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        {this.state.page == 0 && this.state.completedData.length < 1 && this.state.loading == false && this.state.loading1 == false ?
          ...
          <FlatList
            ...
            onEndReached={() => {
              AsyncStorage.getItem("pagelimit").then((pagelimit) => {
                if (pagelimit) {
                  if (this.state.completedData.length >= pagelimit) {
                    this.handleLoadMore();
                  }
                }else{
                  this.handleLoadMore();
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    NetInfo.fetch().then(state => {
                      if (state.isConnected) {
                        this.props.navigation.navigate('EmpCompleted', { item: item })
                      } else {
                        Toast.show(
                          'Please Check your internet connection',
                          Toast.SHORT,
