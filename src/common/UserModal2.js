...
<View>
        ...
        <TouchableOpacity activeOpacity={0.7}
          style={[style, {flexDirection:'row',  paddingHorizontal:20,width:'100%'}]}
          onPress={() => this.props.onSelect(id, so_no,)}
        >
        <Text style={[textStyle, {fontFamily: Fonts.regular}]}>{so_no}</Text>
        {/* <Text style={[textStyle, {fontFamily: Fonts.regular}]}>{phone}</Text> */}
  ...
  onFilterChange = (text) => {
    const { options } = this.props
      ...
      const filtered = (!filter.length)
        ? options
        : options.filter(({ searchKey, so_no, id }) => (
          0 <= so_no.toLowerCase().indexOf(filter) ||
